...
internal sealed class AutomaticNegativeCacheHarness : IAsyncDisposable
{
    ...
    private AutomaticNegativeCacheHarness(WebApplication app, string directory, QuotaSource transport, ITestOutputHelper output)
    {
        _output = output;
        _app = app;
    ...
    public static async Task<AutomaticNegativeCacheHarness> CreateAsync(ITestOutputHelper output)
    {
        ...
        try
        {
            ...
            var rig = new AutomaticNegativeCacheHarness(app, directory, transport, output);
            ...
            await rig.Db.SaveChangesAsync();
            rig.Event = new Event { Title = "Belgian Grand Prix", Sport = "Motorsport", ExternalId = "",
            ...
            rig.Indexer = new Indexer { Name = "Cache source", Type = IndexerType.Newznab,
                Url = transport.Url, ApiPath = "/api", QueryLimit = 30, RequestDelayMs = 20, ApiKey = "fixture", Categories = new List<string> { "5060" },
                Enabled = true, EnableAutomaticSearch = true, EnableInteractiveSearch = false, EnableRss = false };
            ...
            rig.Db.IndexerStatuses.Add(new IndexerStatus { IndexerId = rig.Indexer.Id,
                QueriesThisHour = 0, GrabsThisHour = 0, HourResetTime = DateTime.UtcNow.AddHours(1) });
            await rig.Db.SaveChangesAsync();
    ...
    public void UseMatchingRelease(string? query = null)
    {
        Transport.UseMatchingRelease(Event);
        Transport.MatchingQuery = query;
    }

    public async Task<Indexer> ReplaceIndexerAsync()
    {
        Indexer.Enabled = false;
        var replacement = new Indexer
        {
            ...
            RequestDelayMs = Indexer.RequestDelayMs,
            ApiKey = Indexer.ApiKey,
            Categories = Indexer.Categories.ToList(),
            Enabled = true,
            EnableAutomaticSearch = true,
            EnableInteractiveSearch = false,
            EnableRss = false
        };
        Db.Indexers.Add(replacement);
        await Db.SaveChangesAsync();
        Db.IndexerStatuses.Add(new IndexerStatus
        {
            IndexerId = replacement.Id,
    ...
    public async Task<Phase> RunAsync(string name, bool automatic)
    {
        ...
        if (automatic)
        {
            ...
            selectedIndexer = result.SelectedIndexer;
        }
        else
        {
        ...
        var phase = new Phase(name, started, ElapsedMilliseconds, body, guids, found, selected, selectedIndexer,
    ...
    internal sealed record Phase(string Name, double StartMilliseconds, double EndMilliseconds, string Body,
        string[] Guids, int Found, string? Selected, string? SelectedIndexer, Attempt[] Attempts, State State, string ContractHash, CacheEntry[] CacheEntries);
    ...
    internal sealed class QuotaSource : IAsyncDisposable
    {
        ...
        public string? MatchingQuery { get; set; }
        ...
        {
            Releases = Array.Empty<ReleaseSearchResult>();
        }

        public void UseMatchingRelease(Event evt)
        {
            Releases = new[]
            {
                new ReleaseSearchResult
                {
                    Title = $"Formula.1.{evt.EventDate:yyyy}.Belgian.Grand.Prix.1080p.WEB-DL.H264-GROUP",
                    Guid = "matching-release",
                    Indexer = "Fixture source",
                    DownloadUrl = Url + "/payload/matching-release.nzb",
                    PublishDate = evt.EventDate.AddHours(3),
                    Size = 4L * 1024 * 1024 * 1024,
                    Protocol = "Usenet",
                    SportarrEventId = "ev-2336155"
                }
            };
        }
        ...
        {
            if (Interlocked.Increment(ref _arrivals) > 32 || context.Request.Method != "GET")
            ...
            else if (mode == "search" && !string.IsNullOrWhiteSpace(text) && eventId == "" && offset == 0)
            {
                var catalogue = MatchingQuery == null || text == MatchingQuery
                    ? Releases : Array.Empty<ReleaseSearchResult>();
