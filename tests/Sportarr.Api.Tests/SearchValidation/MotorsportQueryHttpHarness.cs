...
internal sealed class MotorsportQueryHttpHarness : IAsyncDisposable
{
    internal static readonly TimeSpan Deadline = TimeSpan.FromSeconds(30);
    private readonly WebApplication _app;
    private readonly string _directory;
    private readonly ITestOutputHelper _output;
    private Task<AutomaticSearchResult>? _operation;
    ...
    private MotorsportQueryHttpHarness(WebApplication app, string directory, Source source, ITestOutputHelper output)
    {
        _app = app; _directory = directory; Transport = source; _output = output;
        Db = Services.GetRequiredService<SportarrDbContext>();
    }

    internal static async Task<MotorsportQueryHttpHarness> CreateAsync(ITestOutputHelper output, string mode = "phrase")
    {
        ...
        try
        {
            ...
            builder.Services.AddSingleton(new SportarrDbContext(new DbContextOptionsBuilder<SportarrDbContext>().UseInMemoryDatabase(directory).Options));
            ...
            var rig = new MotorsportQueryHttpHarness(app, directory, source, output);
    ...
    internal async Task<AutomaticSearchResult> AutomaticAsync(int? qualityProfileId = null)
    {
        _operation = Services.GetRequiredService<AutomaticSearchService>().SearchAndDownloadEventAsync(Event.Id, qualityProfileId ?? ProfileId);
        var result = await _operation.WaitAsync(Deadline);
        _output.WriteLine(System.Text.Json.JsonSerializer.Serialize(new
        { result.Success, result.SelectedRelease, result.Message, result.ReleasesFound, Transport.DescriptorAttempts, Transport.DescriptorGuids }));
        return result;
    }
