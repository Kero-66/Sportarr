...
internal sealed class UnknownTailCacheHarness : IAsyncDisposable
{
    ...
    public const string Query = "Belgian Grand Prix";
    public double ElapsedMilliseconds => _clock.Elapsed.TotalMilliseconds;

    private UnknownTailCacheHarness(WebApplication app, string directory, QuotaSource transport, ITestOutputHelper output)
    {
        _output = output;
        _app = app;
    ...
    public static async Task<UnknownTailCacheHarness> CreateAsync(ITestOutputHelper output, bool pagedOffers = true, int initialQueries = 0)
    {
        ...
        try
        {
            ...
            var rig = new UnknownTailCacheHarness(app, directory, transport, output);
            ...
            var league = new League { Name = "Formula 1", Sport = "Motorsport", Monitored = true,
                RootFolderId = root.Id, QualityProfileId = rig.Profile.Id, SearchQueryTemplate = Query };
            ...
            await rig.Db.SaveChangesAsync();
            rig.Queries = rig.Services.GetRequiredService<EventQueryService>().BuildEventQueries(rig.Event, null, rig.Event.League!.SearchQueryTemplate).ToArray();
            Assert.Equal(new[] { Query }, rig.Queries);
    ...
    private async Task<CacheEntry[]> ReadCacheAsync(bool automatic)
    {
        ...
        return keys.Select(key =>
        {
            ...
            return new CacheEntry(key, entry?.RawReleases.Select(row => row.Guid).OrderBy(guid => guid).ToArray(), entry?.LifetimeSeconds, entry == null ? null : JsonSerializer.Serialize(entry));
        }).ToArray();
    }
    ...
    internal sealed record CacheEntry(string Key, string[]? Guids, int? LifetimeSeconds, string? MetadataJson);
        ...
        private async Task RespondAsync(HttpContext context)
        {
            ...
            else if (mode == "search" && !string.IsNullOrWhiteSpace(text) && eventId == _eventId && offset >= 0 && offset <= 4)
            {
                ...
                xml = new XElement("rss", new XAttribute(XNamespace.Xmlns + "newznab", ns), new XElement("channel",
                    new XElement(ns + "response", new XAttribute("offset", offset)),
