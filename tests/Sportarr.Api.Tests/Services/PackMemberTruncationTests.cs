...
public class PackMemberTruncationTests(Xunit.Abstractions.ITestOutputHelper output)
{
    [Fact]
    public async Task ShorterPersistedMemberCannotAuthorizeSharedCategoryOrClientRemoval()
    {
        ...
        var file = await rig.Db.EventFiles.SingleAsync(x => x.EventId == rig.Events[0].Id);
        var categoriesBefore = rig.CategoryCalls; var removalsBefore = rig.RemoveCalls;
        Assert.Equal(0, categoriesBefore); Assert.Equal(0, removalsBefore);
        await File.WriteAllBytesAsync(file.FilePath, new byte[1024]);
        Assert.True(File.Exists(file.FilePath)); Assert.Equal(1024, new FileInfo(file.FilePath).Length);
        var persisted = await rig.Db.EventFiles.AsNoTracking().SingleAsync(x => x.Id == file.Id);
        Assert.True(persisted.Exists); Assert.Equal(4096, persisted.Size);
        Assert.Equal(DownloadStatus.Imported, rig.Owners[0].Status);
        await rig.MonitorCompletedAsync(1);
        output.WriteLine(System.Text.Json.JsonSerializer.Serialize(new { phase = "truncated-member", storedLength = persisted.Size,
            actualLength = new FileInfo(file.FilePath).Length, categoriesBefore, removalsBefore,
            categoriesAfter = rig.CategoryCalls, removalsAfter = rig.RemoveCalls,
            actions = rig.Transport.Requests.Where(x => x.Path.EndsWith("setCategory") || x.Path.EndsWith("delete"))
                .Select(x => new { x.Method, x.Path, x.Form }).ToArray() }));
        Assert.Equal(categoriesBefore, rig.CategoryCalls); Assert.Equal(removalsBefore, rig.RemoveCalls);
