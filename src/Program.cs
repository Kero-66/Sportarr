...
static async Task<bool> ShouldExposeApiKeyAsync(HttpContext context, SportarrDbContext db)
{
    ...
    if (appSettings != null)
    {
        try
        {
            // Normalized like the auth middleware: config.xml carries
            // PascalCase and the startup sync stores lowercase, so the
            // comparisons below only work on lowercased values.
            var securitySettings = JsonSerializer.Deserialize<SecuritySettings>(appSettings.SecuritySettings);
            authMethod = (securitySettings?.AuthenticationMethod ?? "none").ToLowerInvariant();
            authRequired = (securitySettings?.AuthenticationRequired ?? "disabledForLocalAddresses").ToLowerInvariant();
        }
        catch
        {
